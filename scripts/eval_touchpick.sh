...
FINETUNED_PATH="${FINETUNED_PATH:-${REPO_ROOT}/checkpoints}/touch_pick/checkpoint"
RECORD_DIR="${RECORD_DIR:-eval_runs/touchpick/results/}"

"${PYTHON_BIN}" eval_scratchpad_vla.py \
            --vla_variant "${VLA_VARIANT}" --start_seed "${START_SEED}" --record_dir "${RECORD_DIR}" \
            --num_episodes "${NUM_EPISODES}" --ep_length "${EP_LENGTH}" --task_description touchpick --num_objects 2 --verbose="${VERBOSE}" \
