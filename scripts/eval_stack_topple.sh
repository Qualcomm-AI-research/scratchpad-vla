...
FINETUNED_PATH="${FINETUNED_PATH:-${REPO_ROOT}/checkpoints}/stack_topple/checkpoint"
RECORD_DIR="${RECORD_DIR:-eval_runs/stack_topple/results/}"

"${PYTHON_BIN}" eval_scratchpad_vla.py \
            --vla_variant "${VLA_VARIANT}" --start_seed "${START_SEED}" --record_dir "${RECORD_DIR}" \
            --num_episodes "${NUM_EPISODES}" --ep_length "${EP_LENGTH}" --task_description stack_topple --num_objects 2 --verbose="${VERBOSE}" \
