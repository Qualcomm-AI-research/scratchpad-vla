...
EP_LENGTH="${EP_LENGTH:-150}"
START_SEED="${START_SEED:-30000}"
VERBOSE="${VERBOSE:-True}"
FINETUNED_PATH="${FINETUNED_PATH:-${REPO_ROOT}/checkpoints}/place_next_restore/checkpoint"
RECORD_DIR="${RECORD_DIR:-eval_runs/place_next_restore/results/}"

"${PYTHON_BIN}" eval_scratchpad_vla.py \
            --vla_variant "${VLA_VARIANT}" --start_seed "${START_SEED}" --record_dir "${RECORD_DIR}" \
            --num_episodes "${NUM_EPISODES}" --ep_length "${EP_LENGTH}" --task_description place_next_restore --num_objects 2 --verbose="${VERBOSE}" \
