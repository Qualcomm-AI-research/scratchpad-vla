...
```bash
docker build -f docker/Dockerfile --tag scratchpad-eval:latest .
```
...
This command starts the shell as a container user inferred from your mounted host
home/cache paths, mounts the Hugging Face cache, and loads optional host shell
customizations from the read-only `/host_home` mount.
